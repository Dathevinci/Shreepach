...
// Keep adult / Hentai titles out of every listing. AniList's `isAdult: false`
// query arg misses some Hentai-genre entries and doesn't apply to airing
// schedules, so we also drop anything carrying the "Hentai" genre here.
const isSafeMedia = (media: any): boolean =>
  !!media &&
  !media.isAdult &&
  !(media.genres || []).some((g: string) => (g || "").toLowerCase() === "hentai");
...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  const query = `
    query ($season: MediaSeason, $year: Int, $nextSeason: MediaSeason, $nextYear: Int, $threeDaysAgo: Int, $now: Int) {
      trending: Page(page: 1, perPage: 20) { media(sort: TRENDING_DESC, type: ANIME, isAdult: false) { ${DASH_FRAGMENT} } }
      airingNow: Page(page: 1, perPage: 20) { media(status: RELEASING, sort: POPULARITY_DESC, type: ANIME, isAdult: false) { ${DASH_FRAGMENT} } }
      thisSeason: Page(page: 1, perPage: 20) { media(season: $season, seasonYear: $year, sort: POPULARITY_DESC, type: ANIME, isAdult: false) { ${DASH_FRAGMENT} } }
      nextSeason: Page(page: 1, perPage: 20) { media(season: $nextSeason, seasonYear: $nextYear, sort: POPULARITY_DESC, type: ANIME, isAdult: false) { ${DASH_FRAGMENT} } }
      upcoming: Page(page: 1, perPage: 20) { media(status: NOT_YET_RELEASED, sort: POPULARITY_DESC, type: ANIME, isAdult: false) { ${DASH_FRAGMENT} } }
      finished: Page(page: 1, perPage: 20) { media(status: FINISHED, sort: POPULARITY_DESC, type: ANIME, isAdult: false, season: $season, seasonYear: $year) { ${DASH_FRAGMENT} } }
  ...
  const gql = `
    query ($page: Int, $search: String, $status: MediaStatus, $season: MediaSeason, $seasonYear: Int, $format: MediaFormat, $genre_in: [String], $sort: [MediaSort]) {
      Page(page: $page, perPage: 20) {
        ...
        media(type: ANIME, isAdult: false, search: $search, status: $status, season: $season, seasonYear: $seasonYear, format: $format, genre_in: $genre_in, sort: $sort) {
          ${DASH_FRAGMENT}
        }
      }
    }
