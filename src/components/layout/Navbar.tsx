        ...
        <div className={`container mx-auto px-4 sm:px-6 flex justify-between items-center transition-all duration-300 ${isScrolled ? 'h-16' : 'h-20 lg:h-24'}`}>
          <div className="flex items-center gap-4 lg:gap-8">
            <Link href="/" className="font-fell font-bold text-xl sm:text-2xl lg:text-3xl text-white tracking-[0.1em] sm:tracking-[0.2em] uppercase flex items-center gap-2 sm:gap-3 drop-shadow-lg shrink-0">
              <img src="/logo.png" alt="Shreepach Logo" className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border border-indigo-400/50 shadow-[0_0_15px_rgba(99,102,241,0.4)] object-cover" />
              <span className="hidden xs:inline-block">SHREE<span className="text-indigo-400 font-black drop-shadow-[0_0_10px_rgba(99,102,241,0.5)]">PACH</span></span>
            ...
            <nav className="hidden lg:flex gap-4 xl:gap-6 font-medium text-sm text-slate-300 shrink-0">
              <Link href="/" className="hover:text-white hover:text-indigo-400 transition whitespace-nowrap">Home</Link>
              <Link href="/novel" className="hover:text-white hover:text-indigo-400 transition whitespace-nowrap flex items-center gap-2">
                <Compass className="w-4 h-4" /> Explore Novels
              </Link>
            ...
            {isLoaded && (
              user ? (
                <div className="hidden lg:flex items-center gap-3 xl:gap-4 relative shrink-0">
                  <Link href={`/user/${user.username}`} className="flex items-center gap-2 text-sm font-bold bg-white/10 hover:bg-white/20 px-3 py-2 xl:px-4 xl:py-2 border border-white/10 rounded-full transition shadow-lg text-white whitespace-nowrap group">
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-xl flex flex-col">
          <div className="flex justify-between items-center p-4 border-b border-white/10">
            <Link href="/" className="font-cinzel font-bold text-2xl tracking-[0.2em] uppercase text-white flex items-center gap-3" onClick={() => setIsMobileMenuOpen(false)}>
              <img src="/logo.png" alt="Shreepach Logo" className="w-8 h-8 rounded-full border border-indigo-400/50 shadow-[0_0_15px_rgba(99,102,241,0.4)] object-cover" />
              SHREE<span className="text-indigo-400 font-black drop-shadow-[0_0_10px_rgba(99,102,241,0.5)]">PACH</span>
            </Link>
          ...
          <div className="flex flex-col p-6 gap-6 text-xl font-bold text-slate-300">
            <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-indigo-400">Home</Link>
            <Link href="/novel" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-indigo-400 flex items-center gap-3">
              <Compass className="w-6 h-6" /> Explore Novels
            </Link>
