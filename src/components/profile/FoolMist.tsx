...
function useFoolCanvas(canvasRef: RefObject<HTMLCanvasElement | null>, active: boolean) {
  useEffect(() => {
    if (!active) return;
...
// ── Profile-level: the Divine Realm, portaled to <body> ─────────────────────

export function FoolCardMist() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  // Portal to <body>: ancestors carry framer transforms which would re-anchor
  // position:fixed to the card — from <body> the mist truly covers the site.
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
  useFoolCanvas(canvasRef, mounted);
