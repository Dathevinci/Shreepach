...
function useEvernightCanvas(canvasRef: RefObject<HTMLCanvasElement | null>, active: boolean) {
  useEffect(() => {
    if (!active) return;
    ...
    const frame = (now: number) => {
      ...
      EVERNIGHT_ANCHORS.forEach((el) => {
        ...
        if (r.width > best && r.bottom > 0 && r.top < H) {
          best = r.width;
          target = { x: r.left + r.width / 2, y: r.top + r.height / 2, r: r.width / 2 };
        }
      });
...
// ── Profile-level: the tranquil night, portaled to <body> ───────────────────

export function EvernightCardNight() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  // Portal to <body>: ancestors carry framer transforms which would re-anchor
  // position:fixed to the card — from <body> the night truly covers the site.
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
  useEvernightCanvas(canvasRef, mounted);
