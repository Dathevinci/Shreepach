 ...
 *
 * Originally the starfield canvas lived INSIDE the profile card, so the card's
 * overflow-hidden clipped the galaxy at the edges and the cover banner sat on
 * top of it — a squished corner smear. Now it follows the same architecture as
 * the other realm effects:
 *
 * - BlackHoleCardRealm (profile-level, portaled to <body>): a full-viewport
 *   canvas (mix-blend-mode: screen) draws the spiral galaxy CENTERED ON THE
 *   AVATAR'S LIVE COORDINATES (getBoundingClientRect per frame), scaled to the
 *   avatar radius — so the accretion spiral sweeps out of the card and across
 *   the page, unclipped. Star trails come from a destination-out fade instead
 *   of a hard clear. A doomed star spirals in, spaghettifies and redshifts.
 *
...
function useBlackHoleCanvas(canvasRef: RefObject<HTMLCanvasElement | null>, active: boolean) {
  useEffect(() => {
    if (!active) return;
    ...
    const frame = (now: number) => {
      ...
      // center the galaxy on the largest visible registered avatar
      let target: { x: number; y: number; r: number } | null = null;
      let best = 0;
      BH_ANCHORS.forEach((el) => {
        const rect = el.getBoundingClientRect();
        if (rect.width > best && rect.bottom > 0 && rect.top < H) {
          best = rect.width;
          target = { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2, r: rect.width / 2 };
        }
      });
      if (!target) {
...
// ── Profile-level: the galaxy, portaled to <body> so it escapes the card ────

export function BlackHoleCardRealm() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
  useBlackHoleCanvas(canvasRef, mounted);
