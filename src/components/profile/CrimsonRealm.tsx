 ...
 *
 * - CrimsonCardRealm (profile-level, portaled to <body> to escape every
...
// The one soul this realm answers to.
export const CRIMSON_CHOSEN = "māna-yood-sushāī";
export function isCrimsonChosen(username?: string | null) {
  return (username || "").toLowerCase() === CRIMSON_CHOSEN;
}
...
function useCrimsonCanvas(canvasRef: RefObject<HTMLCanvasElement | null>, active: boolean) {
  useEffect(() => {
    if (!active) return;
...
// ── Profile-level: the realm itself, portaled to <body> ─────────────────────

export function CrimsonCardRealm() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  // Portal to <body>: ancestors carry framer transforms which would re-anchor
  // position:fixed to the card — from <body> the realm truly covers the site.
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
  useCrimsonCanvas(canvasRef, mounted);
