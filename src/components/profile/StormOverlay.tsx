 ...
 *
 * - TempestCardStorm (profile-level, portaled to <body>):
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      COLLIDERS.forEach((el) => {
        ...
        if (r.width > 8 && r.bottom > 0 && r.top < H) {
...
// ── Profile-level: the full storm, portaled to <body> ───────────────────────

export function TempestCardStorm() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const wrapperRef = useRef<HTMLDivElement>(null);
  // Portal to <body>: ancestors of the profile card carry framer transforms,
  // which would re-anchor position:fixed to the card instead of the viewport.
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
  useStormCanvas(canvasRef, wrapperRef, mounted);

  return (
    <>
      {/* the storm rages over the WHOLE site while this profile is open */}
      {mounted &&
        createPortal(
          <div
            ref={wrapperRef}
